#!/usr/bin/env python2
#
# wtc1_bach_central_dots_score_visualiser.py
#
#  Copyright 2014-2022  Randall E. Cone.  All Rights Reserved.
#
...
outputfile = 'wtc1_bach_dots_visualisation.svg'


# File containing musicxml information:
inputfiles = [
    {'filename': 'musicxml/BWV846-Prelude-Breitkopf-SATB.musicxml',
     'title': 'Prelude in',
     'bwv': 'BWV 846',
     'key': 'C Major'},
    {'filename': 'musicxml/BWV846-Fugue-Breitkopf-SATB.musicxml',
     'title': 'Fugue in',
     'bwv': 'BWV 846',
     'key': 'C Major'},
    {'filename': 'musicxml/BWV847-Prelude-Breitkopf-SATB.musicxml',
     'title': 'Prelude in',
     'bwv': 'BWV 847',
     'key': 'C Minor'},
    {'filename': 'musicxml/BWV847-Fugue-Breitkopf-SATB.musicxml',
     'title': 'Fugue in',
     'bwv': 'BWV 847',
     'key': 'C Minor'},
    {'filename': 'musicxml/BWV848-Prelude-Breitkopf-SATB.musicxml',
     'title': 'Prelude in',
     'bwv': 'BWV 848',
     'key': 'C&#9839; Major'},   # sharp symbol
    {'filename': 'musicxml/BWV848-Fugue-Breitkopf-SATB.musicxml',
     'title': 'Fugue in',
     'bwv': 'BWV 848',
     'key': 'C&#9839; Major'},   # sharp symbol
    #	       {'filename': 'musicxml/BWV849-Prelude-Breitkopf-SATB.musicxml',
    #                'title': 'Prelude in',
    #                'bwv': 'BWV 849',
    #                'key': 'C&#9839; Minor'},   # sharp symbol
    #               {'filename': 'musicxml/BWV849-Fugue-Breitkopf-SATB.musicxml',
    #                'title': 'Fugue in',
    #                'bwv': 'BWV 849',
    #                'key': 'C&#9839; Minor'},   # sharp symbol
...
colors = ['#6975A6', '#F3E96B', '#F28A30',
          '#F03737', '#7e37f0']   # pastel sunset 2 purply
...
title_string = ('<text xml:space="preserve" fill="' + str(font_color) + '" font-family="Didot"'         # good'
                ...
                + ' x="' + str(bg_width/2.0) + '" y="' + \
                str(0.6*mainy + 2.0*top_margin) + '">'
                + 'The Well-Tempered Clavier - Book I' + '</text>\n')


#  REMOVE temporarily for the printing process, when needed,  whereby the JS Bach signature png is
...
title_string += ('<text xml:space="preserve" fill="' + font_color + '" font-family="Times"'
                 ...
                 + ' x="' + str(top_margin) + '" y="' +
                 str(bg_height - 0.8*border_margin) + '">'
...
for filedict in inputfiles:
    ...
    # Gabriel Editted this, it was reversed(SP.parts.keys()) but it gave me an error saying object keys not reversable
    for partkey in reversed(SP.parts.keys()):
        ...
        if (not SP.parts[partkey]['is_empty']):
            ...
            # 3.5 octaves above and below middle C at center of image, 8 notes per octave
            note_height = score_height/(7.0 * 8.0)
            ...
            c_string = ('<line x1="' + str(xmargin) + '" y1="' + str(center_line_y) + '" '
                        + 'x2="' + str(xmargin+c_line_width) +
                        '" y2="' + str(center_line_y) + '" '
                        + 'stroke="' + font_color + '" '
            ...
            left_string = ('<text xml:space="preserve" fill="' + font_color + '" font-family="Serif"'
                           + ' stroke="' + str(font_color) +
                           '" stroke-width="1.0"'
                           + ' font-size="' +
                           str(font_size) + '" text-anchor="left"'
                           + ' x="' + str(xmargin/3.0) + '" y="' +
                           str(center_line_y - font_size/2.0) + '">'
                           + filedict['key'] + '</text>\n')
            left_string += ('<text xml:space="preserve" fill="' + font_color + '" font-family="Serif"'
                            + ' stroke="' + str(font_color) +
                            '" stroke-width="1.0"'
                            ' font-size="' +
                            str(font_size) + '" text-anchor="center"'
                            ' x="' + str(xmargin/3.0) + '" y="' +
                            str(center_line_y + font_size - 5.0) + '">'
                            + filedict['title'][0:-3] + '</text>\n')
            ...
            right_string = ('<text xml:space="preserve" fill="' + font_color + '" font-family="Serif"'
                            ...
                            ' font-size="' +
                            str(font_size) + '" text-anchor="left"'
                            ' x="' + str(xmargin + c_line_width + xmargin *
                                         0.25) + '" y="' + str(center_line_y) + '">'
                            + filedict['bwv'] + '</text>\n')
                ...
                if ((note.tiestart) and (note.voice == 1)):
                    ...
                    # Normal Note:
                        ...
                        if note.pitch_class:
                            ...
                            notey = centery - note_height * \
                                (note.piano_note_number -
                                 center_piano_note) * 8.0/12.0
